...
import { BienCategoria } from 'apps/models/src/lib/ElementosEconomicos/Bien/BienCategoria';
...
export class BienCategoriaService {
    ...
    getCollection()
    {
        return this.httpService.post<BienCategoria[]>({
    ...
    getItem( bienCategoria: BienCategoria )
    {
        return this.httpService.post<BienCategoria>({
            service: this.serviceName,
            method: 'getItem',
            values: { bienCategoria }
        })
        .pipe( map( item => new BienCategoria( item ) ) )
    }


    createItem( bienCategoria: BienCategoria )
    {
        return this.httpService.post<BienCategoria>({
            service: this.serviceName,
            method: 'createItem',
    ...
    updateItem( bienCategoria: BienCategoria )
    {
        return this.httpService.post<BienCategoria>({
            service: this.serviceName,
            method: 'updateItem',
    ...
    deleteItem( bienCategoria: BienCategoria )
    {
    ...
    tableBindingProperties: TableComponentVm<BienCategoria>['bindingProperties'] = [
    ...
    {
        return overlayService.open( TableComponent<BienCategoria> ).pipe(
            tap( c => {
                ...
                c.sub.add( c.onAddItem.subscribe( e => 
                    this.openObjectComponent( c.store, c.store.storeFromThis( () => new BienCategoria() ) )
    ...
    {
        return this.modalService.open( TableComponent<BienCategoria> ).pipe(
            tap( c => {
    ...
    openObjectComponent( parentStore: ComponentStore<BienCategoria[]>, store: ComponentStore<BienCategoria> )
    {
        return this.modalService.open( ObjectComponent<BienCategoria> )
        .pipe(
            tap( c => {
