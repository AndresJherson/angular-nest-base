...
import { ServicioCategoria } from 'apps/models/src/lib/ElementosEconomicos/Servicio/ServicioCategoria';
...
export class ServicioCategoriaService {
    ...
    getCollection()
    {
        return this.httpService.post<ServicioCategoria[]>({
            service: this.serviceName,
            method: 'getCollection',
        })
        .pipe( map( data => data.map( item => new ServicioCategoria( item ) ) ) )
    }


    getItem( servicioCategoria: ServicioCategoria )
    {
        return this.httpService.post<ServicioCategoria>({
            service: this.serviceName,
            method: 'getItem',
            values: { servicioCategoria }
        })
        .pipe( map( item => new ServicioCategoria( item ) ) )
    }


    createItem( servicioCategoria: ServicioCategoria )
    {
        return this.httpService.post<ServicioCategoria>({
            service: this.serviceName,
            method: 'createItem',
    ...
    updateItem( servicioCategoria: ServicioCategoria )
    {
        return this.httpService.post<ServicioCategoria>({
            service: this.serviceName,
            method: 'updateItem',
    ...
    deleteItem( servicioCategoria: ServicioCategoria )
    {
    ...
    tableBindingProperties: TableComponentVm<ServicioCategoria>['bindingProperties'] = [
    ...
    {
        return overlayService.open( TableComponent<ServicioCategoria> ).pipe(
            tap( c => {
                ...
                c.sub.add( c.onAddItem.subscribe( e => 
                    this.openObjectComponent( c.store, c.store.storeFromThis( () => new ServicioCategoria() ) )
    ...
    {
        return this.modalService.open( TableComponent<ServicioCategoria> ).pipe(
            tap( c => {
    ...
    openObjectComponent( parentStore: ComponentStore<ServicioCategoria[]>, store: ComponentStore<ServicioCategoria> )
    {
        return this.modalService.open( ObjectComponent<ServicioCategoria> )
        .pipe(
            tap( c => {
